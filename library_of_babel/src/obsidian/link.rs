...
impl Link {
    ...
    pub fn parse_links(string: &str, files: &[&File]) -> Vec<Link> {
        lazy_static! {
            static ref match_links: Regex =
    ...
    /// `Some` if this is an embed of an image file that exists.
    pub fn embedded_image<'vault>(&self, vault: &'vault Vault) -> Option<&'vault File> {
        if !self.is_embed {
            return None;
        }

        let vault_item_id = self.vault_item_id.as_ref()?;

        // We call expect() instead of using the `?` operator because if
        // self.vault_item_id is Some, that implies that the file
        // should exist in the vault.
        let file = vault.file(vault_item_id).expect("File not found.");
    ...
    /// `Some` if this is an embed of a file that exists.
    pub fn embedded_file<'vault>(&self, vault: &'vault Vault) -> Option<&'vault File> {
        if !self.is_embed {
            return None;
        }
